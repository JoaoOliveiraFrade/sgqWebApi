...
from
	...
	where 
		...
		and ct.ciclo like ('%@mmm/@yy%')
		and @systemConditionCt
		and Status_Exec_CT <> 'CANCELLED'
		and substring(dt_planejamento,7,2) + substring(dt_planejamento,4,2) + substring(dt_planejamento,1,2) <= convert(varchar(6), getdate(), 12)
		and dt_planejamento <> ''
	...
	where
		...
		and @systemConditionCt
		and Status_Exec_CT <> 'CANCELLED'
		and Evidencia_Validacao_Tecnica = 'VALIDADO'
	...
	where
		...
		and ct.ciclo like ('%@mmm/@yy%')
		and @systemConditionCt
		and Status_Exec_CT = 'PASSED'
		and UAT = 'SIM'
		and Evidencia_Validacao_Cliente = 'VALIDADO'
	) Aux
group by
	date
order by
