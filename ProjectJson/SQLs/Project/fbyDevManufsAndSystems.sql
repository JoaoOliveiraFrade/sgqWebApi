...
where
	(case when IsNull(cts.fabrica_desenvolvimento,'') <> '' then cts.fabrica_desenvolvimento else 'NÃO IDENTIFICADA' end) in (@devManufs)
	and (case when IsNull(cts.Sistema,'') <> '' then cts.Sistema else 'NÃO IDENTIFICADO' end) in (@systems)
...
--	(
--		select distinct
--			(case when IsNull(cts.fabrica_desenvolvimento,'') <> '' then cts.fabrica_desenvolvimento else 'NÃO IDENTIFICADA' end) as devManuf
--			,(case when IsNull(cts.Sistema,'') <> '' then cts.Sistema else 'NÃO IDENTIFICADO' end) as system
--			,cts.Subprojeto as subproject
--			,cts.Entrega as delivery
--		from
--			ALM_CTs cts with (NOLOCK) 
