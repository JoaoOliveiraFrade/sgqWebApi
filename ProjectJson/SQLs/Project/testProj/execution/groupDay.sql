...
from
	...
	select 
		substring(date,5,2)+'-'+substring(date,3,2)+'-'+substring(date,1,2) as date, 
		active, activeTechnique, activeClient, planned, realized, productivity, approvedTI, approvedUAT
	from
		(	
			select 
				left((select
					max(substring(dt_alteracao,7,2)+substring(dt_alteracao,4,2)+substring(dt_alteracao,1,2))
				from 
