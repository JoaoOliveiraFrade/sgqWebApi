...
where 
	...
	and (case when IsNull(df.fabrica_desenvolvimento,'') <> '' then df.fabrica_desenvolvimento else 'N/A' end) in (@selectedDevManuf)
	and (case when IsNull(df.sistema_defeito,'') <> '' then df.sistema_defeito else 'N/A' end) in (@selectedSystem)
group by
